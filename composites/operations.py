...
def __or__(self, object=slice(None)):
    ...
    """
    return ifnot(self[:])[
        object]  # There is no reasonable way to make this an attribute?
...
def __xor__(self: 'λ', object: (slice, Exception)=slice(None)) -> 'λ':
    ...
    FalseException("TypeError:partial_attribute(<method 'upper' of 'str' objects>)\\ndescriptor 'upper' requires a 'str' object but received a 'int'",)
    """
...
[
    setattr(
        ...
        partialmethod(
            op_attr,
            dunder(attr))) or setattr(
        object,
        dunder(
            'r' + attr),
        partialmethod(
            right_attr,
            dunder(attr))) for attr in [
        'mul',
        'add',
        'rshift',
        'sub',
...
[setattr(object, key, getattr(object, dunder(other))) for key, other in zip(
    ('do', 'excepts', 'instance'), ('lshift', 'xor', 'pow')) for object in [factory, composite]]
