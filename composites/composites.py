...
# # Composites
#
# Composites are functional programming-style objects in Python.
# Composites rely on `toolz` and Python builtins to compose higher-order
# functions. Effectively, composites are lists and dicts with call
# methods.
...
# CallableList subclasses `UserList` to manipulate compositions with a
# familar `api`; this approach provides total ordering.
...
class CallableList(UserList):
    ...
    def __call__(self, *args, **kwargs) -> Iterator:
        for object in self:
            args, kwargs = [object(*args, **kwargs)
...
class compose(CallableList):
    ...
    def __call__(self, *args, **kwargs) -> Iterator:
        ...
        while True:
            try:
                ...
                if not any(
                        line in callable for line in e.args[0].splitlines()):
                    e.args = callable + '\n' + e.args[0], *e.args[1:]
                raise e
    ...
    def __init__(self, data=None):
        super().__init__(
            data is not None and (
                not isiterable(data) or isinstance(
                    data, str)) and [data] or data or list())
    ...
    def __setstate__(self, state):
        for attr, value in zip(reversed(self.__slots__), reversed(state)):
...
class factory(compose):
    ...
    def __getitem__(self, attr):
        if isinstance(self.args, tuple) and isinstance(self.kwargs, dict):
            attr = attr == slice(None) and abs(self) or partial(
                attr, *self.args, **self.kwargs)
...
# __juxt__ applies the same arguments to a list of functions.  The
# composite version adds the ability use mapping objects and control the
# return class.
...
class juxt(composite):
    ...
    def __iter__(self):
        for callable in self.data:
            if not isinstance(callable, (str, compose)
                              ) and isiterable(callable):
    ...
    def __iter__(self):
        for i, value in enumerate(super().__iter__()):
            if callable(value) and i is 0:
                value = flipped(value.func,
                                *value.args,
                                **value.keywords) if isinstance(value,
                                                                partial) else flipped(value)
            yield value
...
class star(composite):
    ...
    def __call__(self, *inputs):
        ...
        [kwargs.update(**input) if isinstance(input, dict)
         else args.extend(input) for input in inputs]
...
# __a__, __an__, __the__, and __λ__ are the main __articles__ used for
# function composition.  They seemed like uncommon namespace choices.
