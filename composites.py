...
class pro(pose):
    ...
    def __call__(pose, *tuple, **dict):
        ...
        return identity(*tuple, **dict) if logic is True else logic
...
if __name__ == '__main__':
    if 'runtime' in sys.argv[-1]:
        ...
        get_ipython().system('source activate p6 && python -m pydoc -w composites')
        get_ipython().system('source activate p6 && pyreverse -o png -pcomposites -fALL composites')
        display.display(display.Image('classes_composites.png'), display.IFrame('composites.html', height=600, width=800))
        
        get_ipython().system('source activate p6 && ipython -m doctest composites.py')
        get_ipython().system('source activate p6 && python -m pydoc -w composites')
    else:
        print('run from cli')
