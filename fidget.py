...
class Composition(Callables):
    ...
    def _rattr_(attr):
        ...
        setattr(
            Composition, right, wraps(getattr(Composition, attr))(caller)
        )
    ...
    for attr, method in iteritems(
        ...
        if getattr(Composition, attr, None) is None:
            if method in (flip,) or imports == 'toolz': 
                pass
            elif method in (methodcaller, itemgetter, attrgetter, not_, truth, abs, invert, neg, pos, index):
...
for attr in ['add', 'sub', 'mul', 'matmul',
             'div', 'truediv', 'floordiv', 'mod', 'lshift', 'rshift', 'and', 'xor', 'or']:
    s = "__{}{}__".format
