...
from toolz.curried import (complement, compose, concat, filter, first, flip, get, groupby, isiterable, identity, keyfilter, map, merge, reduce, valfilter)
from six.moves.builtins import hasattr, getattr, isinstance, issubclass, setattr
from operator import (abs, attrgetter, eq, index, invert, itemgetter, methodcaller, neg, not_, pos, truth)
...
for name, func in (('Flips', flipped), ('Stars', starred), ('Does', do), ('Maps', map), ('Filters', filter), ('Groups', groupby), ('Reduces', reduce)):
...
composables.extend(attrgetter('keyfilter', 'keymap', 'valfilter', 'valmap', 'itemfilter', 'itemmap')(import_module('toolz')))
    ...
    class Magics(Calls):
        def _decorate_(self, function):
            return get_ipython().register_magic_function(
                calls[does[stars.second()[function.function]]][None], magic_kind='cell', magic_name=self.args[0])
except: 
    pass
